...
describe('fetchDeliveries', function() {
    it('returns more than 1 delivery object and more than 1 order item', async () => {
        const deliveries = await fetchDeliveries('2022-01-02');
    
        expect(deliveries.length).toBeGreaterThan(1);
        expect(deliveries[0].orderItems.length).toBeGreaterThan(1);
    });

    it('generates sequential ids for deliveries and orderItems', async () => {
        const deliveries = await fetchDeliveries('2022-01-02');

        // not sure what is going on with these two type issues; I had to timebox this part
        expect((deliveries[0].id)).toBeLessThan((deliveries[1].id) as any);
        expect(deliveries[0].orderItems[0].id).toBeLessThan((deliveries[0].orderItems[1].id) as any);
    });
    
    it('returns a set of valid delivery objects when calling fetchDeliveries with a day key in YYYY-MM-DD', async () => {
        const deliveries = await fetchDeliveries('2022-01-02');
    ...
    it('returns the same set of delivery objects when called twice with the same day key in YYYY-MM-DD', async () => {
        const deliveries1 = await fetchDeliveries('2022-01-02');
        const deliveries2 = await fetchDeliveries('2022-01-02');
    ...
    it('returns a new set of delivery objects when called twice with different day keys in YYYY-MM-DD', async () => {
        const deliveries1 = await fetchDeliveries('2022-01-02');
        const deliveries2 = await fetchDeliveries('2022-01-03');
...
describe('storeDeliveries', () => {
    it('properly stores the new state of the deliveries to be fetched later', async () => {
        const deliveries1 = await fetchDeliveries('2022-01-02');
